...
pygame.mixer.music.load(
    'Doom Eternal OST - The Only Thing They Fear Is You (Mick Gordon) [Doom Eternal Theme].mp3')
...
def get_user_input():
    ...
    while running:
        ...
        SCREEN.fill(BLACK)
        draw_text_input_prompt("Guess a number:")
...
def draw_array(array, y_pos, selected_number=None):
    ...
    for i, val in enumerate(array):  # enumerate keeps track of number of iterations
        color = GREEN if val == selected_number else RED
        pygame.draw.rect(SCREEN, color, (start_x + i * bar_width,
                         y_pos - val * 5 + 350, bar_width, val * 5))

    # for i, val in enumerate(array):  # enumerate keeps track of number of iterations
    #     color = BLUE if val == user_number else RED
    #     pygame.draw.rect(SCREEN, color, (start_x + i * bar_width,
    #                      y_pos - val * 5 + 350, bar_width, val * 5))
...
def draw_points(points):
    ...
    SCREEN.blit(point_text, (1250, 10))
...
def main():
    ...
    while running:
        array_size = 10 * level
        array = [random.randint(1, 10 * level) for _ in range(array_size)]
        sorted_array = array[:]
        ...
        while True:
            ...
            if sorted:
                SCREEN.fill(BLACK)
                draw_array(sorted_array, SCREEN.get_height() //
                           2 - 200, target)
                draw_level(level)
                draw_points(points)
                ...
                if found:
                    ...
                    search_result_text = FONT.render(
                        f'The target was {target}', True, GREEN if search_result != -1 else WHITE)
                    search_result_text_rect = search_result_text.get_rect(
                        center=(SCREEN.get_width() // 2, SCREEN.get_height() - 250))
                    SCREEN.blit(search_result_text, search_result_text_rect)
                    user_number = FONT.render(
                        f'Your number was {bet_number}', True, GREEN if search_result != -1 else WHITE)
                    user_number_rect = user_number.get_rect(
                        center=(SCREEN.get_width() // 2, SCREEN.get_height() - 200))
                    SCREEN.blit(user_number, user_number_rect)

                    if not correct_guess:
                        if bet_number == target:
                            points += 100
                            level += 2
                            pygame.mixer.Sound.play(win_sound)
                            result_text = FONT.render(
                                f'Congratulations! You won the bet! +100 points AND you skip an level?!', True, GREEN)
                            correct_guess = True
                        # elif abs(bet_number - search_result) <= 5 or abs(bet_number - search_result) >= 5:
                        elif bet_number + 1 == target or bet_number + 2 == target or bet_number + 3 == target or bet_number + 4 == target or bet_number + 5 == target:
                            points += 10
                            level += 1
                            pygame.mixer.Sound.play(win_sound)
                            result_text = FONT.render(
                                f'Close enough, +10 points', True, GREEN)
                            # BET NUMBER USER INPUT
                            # TARGET THE RANDOMLY CHOSEN TARGET
                            #
                            correct_guess = True
                        elif bet_number - 1 == target or bet_number - 2 == target or bet_number - 3 == target or bet_number - 4 == target or bet_number - 5 == target:
                            points += 10
                            ...
                            if points <= 0:
                                result_text = FONT.render(
                                    f'You Lose! The Casino now owns your soul!', True, RED)
                            else:
                                result_text = FONT.render(
                                    f'Sorry, you lost the bet. Half your points have been deducted', True, RED)

                        result_text_rect = result_text.get_rect(
                            center=(SCREEN.get_width() // 2, SCREEN.get_height() - 50))
                        SCREEN.blit(result_text, result_text_rect)
                        points_text = FONT.render(
                            f'Your points: {points}', True, WHITE)
                        ...
                        if points <= 0:
                            game_over_text = FONT.render(
                                f'Game Over! You ran out of points.', True, RED)
